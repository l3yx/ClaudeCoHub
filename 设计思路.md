...
6. 周期任务的配置还是需要持续化的，但不用数据库，用~/workdir/xxxx/下面创建 .claudecohub 什么目录，然后里面放schedules.yaml
7. 所有用户的claud code 都走同一个llm api，默认在~/.claude/settings.json 配置好

后续补充：
* 会话状态统一定义为：offline ，working，idle
* 即使只创建过一个会话，后续都是resume这个会话，tree ~/.claude/projects/-Users-leixiao-workdir-123123/ 目录下依旧会很很多 [uuid].jsonl,需要根据内容过滤出真实有效的最初会话，可以简单过滤：包含 'sessionId' 关键字
* 周期任务schedules.yaml配置放到 ~/.claude/claudecohub/ 目录, 然后每个任务的元素有 name，content，cron，enabled，workdir
* ~/.claude/claudecohub/ 目录下再搞个配置文件存放账号和密码，仅有这个配置里的账号可以登陆。每个账号元素有 uid, username, password. 登陆用uid和password，username用作以后显示
* 要有一个全局dashboard页面，显示所有用户的会话状态和周期任务